 ...
 */
const stringIsValid = (x: string) => x && typeof x !== "undefined" && x.length <= CONFIG.MAX_PAS_LENGTH && x.length >= CONFIG.MIN_PAS_LENGTH
...
const getObjectLocalStorage = (hash: string, prefix: string = "0xen") => {
    ...
    for (let index = 0; index < localStorage.length; index++) {
        ...
        if (x?.startsWith(prefix)) {
            ...
            value = Buffer.from(AES.decrypt(value, hash).toString(), "hex").toString()
            if (value === "true") value = true
            else if (value === "false") value = false
            else if (value.startsWith("[") && value.endsWith("]") || value.startsWith("{") && value.endsWith("}")) value = JSON.parse(value)
            const y = Object.keys(value)
            if (y.includes("seconds") && y.includes("nanoseconds"))
                obj[x.replace(prefix, "")] = (new Timestamp(value.seconds, value.nanoseconds)).toDate()
            else
            obj[x.replace(prefix, "")] = value
        }
    }
    return obj
}
